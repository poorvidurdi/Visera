...
/**
 * [DEV ONLY] Directly overrides a symbol's price in the internal store and
 * emits a synthetic tick, bypassing the random-walk simulation.
 *
 * Optionally accepts `high52` and `low52` overrides so callers can also
 * force 52-week boundary crossings without waiting for the random walk to drift there.
 *
 * @param {string} symbol - Ticker symbol (case-insensitive).
 * @param {number} price  - New price to inject.
 * @param {{ high52?: number, low52?: number }} [opts]
 * @returns {{ symbol: string, price: number, high52: number, low52: number }}
 *          The snapshot that was emitted, or null if the symbol is unknown.
 */
function forcePrice(symbol, price, opts = {}) {
  ...
  // Optional boundary overrides (useful for forcing 52w high/low crossings)
  if (opts.high52 !== undefined) data.high52 = Number(opts.high52);
  if (opts.low52  !== undefined) data.low52  = Number(opts.low52);

  // Update 52-week bounds from the new price (mirrors normal tick logic)
  data.high52 = Math.max(data.high52, newPrice);
  data.low52  = Math.min(data.low52,  newPrice);

  // Record return in rolling window so volatility stats stay consistent
  data.rollingReturns.push(actualReturn);
